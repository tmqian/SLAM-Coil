...
_, idx_ends = get_coil_scoord(rt.coils, axis_path, 'Blue')

B_mag_center1 = B_mag[idx_ends[0]:idx_ends[1]]
s_coord_center1 = s_coord[idx_ends[0]:idx_ends[1]]

peak_idx, _ = find_peaks(B_mag_center1)
trough_idx, _ = find_peaks(-B_mag_center1)

ripple = (np.max(B_mag_center1[peak_idx]) - np.min(B_mag_center1[trough_idx])) / np.max(B_mag_center1) # quantity to minimize
print(f"Ripple for Blue Center 1 coil: {ripple:.4%}")
...
def B_field(s,currents):
    ...
    _, idx_ends = get_coil_scoord(rt.coils, axis_path, 'Lani')
    ...
    B_region = B_mag[idx_ends[0]:idx_center]
    ...
    if len(peak_idx) > 0 and len(trough_idx) > 0:
        new_ripple = (np.max(B_mag_center[peak_idx]) - np.min(B_mag_center[trough_idx])) / np.max(B_mag_center)
    else:
        new_ripple = (np.max(B_mag_center) - np.min(B_mag_center)) / np.max(B_mag_center)  # Flat field, no ripple
    
    if round(ripple, 4) != round(new_ripple, 4):
        print(f"Ripple for Center Coils: {new_ripple:.4%}")
        ripple = new_ripple

    s_coord = s_coord_center
    return B_region
...
if plot:
    ...
    result = least_squares(residuals, initial_guess, bounds=(lower_bounds, upper_bounds), args=(0.25,), callback=plot_callback)

else:   
    result = least_squares(residuals, initial_guess, bounds=(lower_bounds, upper_bounds), args=(0.25,))
