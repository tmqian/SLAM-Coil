...
class Coil:
    ...
    def __init__(self, Xc=1, Yc=1, angle=90, type=None):
        ...
        if self.type not in models:
            raise ValueError(
                f"Unknown coil type '{self.type}'. Add it to coil-model.csv."
            )
        ...
        self._radial_filaments = self._build_midpoints(
            self.ID / 2, self.OD / 2, self.nr
        )
        self._axial_filaments = self._build_midpoints(
            -self.DZ / 2, self.DZ / 2, self.nz
        )
            ...
            for z_offset in self._axial_filaments:
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    cos_phi = np.divide(
                        rel_local[:, 0],
                        rho,
                        out=np.zeros_like(rel_local[:, 0]),
                        where=rho != 0,
                    )
                    sin_phi = np.divide(
                        rel_local[:, 1],
                        rho,
                        out=np.zeros_like(rel_local[:, 1]),
                        where=rho != 0,
                    )
    ...
    def _loop_field(radius, rho, z, current):
        ...
        if np.any(on_axis):
            Bz[on_axis] = (
                MU0 * current * radius**2 / (2 * (radius**2 + z[on_axis] ** 2) ** 1.5)
            )
        return Br, Bz
...
def contour_plot(
    ...
    contour = ax.contourf(
        xs, ys, Bplane, levels=levels, cmap="jet", alpha=0.7, extend=extend
    )
...
def field_streamplot(
    fig, ax, coils, show_labels=True, length_units="m", field_units="T", color=""
):
